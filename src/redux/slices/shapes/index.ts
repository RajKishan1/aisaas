// import { createEntityAdapter, EntityState, nanoid, PayloadAction, createSlice } from "@reduxjs/toolkit";
// import type { Point } from "../viewport";
// import type { Point } from "@/type/viewport";
...
//  fontStyle: p.fontStyle ?? 'normal', textAlign: p.textAlign ?? 'left', textDecoration: p.textDecoration ?? 'none', lineHeight: p.lineHeight ?? 1.2,
...
// },
// addText(state, action: PayloadAction<Parameters<typeof makeArrow>[0]>) {
...
import type { Point } from "../viewport"; // keep single correct import
...
const makeGeneratedUI = (p: {
  ...
  w: p.w,
  h: p.h,
  uiSpecData: p.uiSpecData ?? null,
  sourceFrameId: p.sourceFrameId,
  isWorkflowPage: p.isWorkflowPage ?? false,
  stroke: p.stroke ?? DEFAULTS.stroke,
  strokeWidth: p.strokeWidth ?? DEFAULTS.strokeWidth,
  fill: p.fill ?? null,
});
